...
\begin{figure}[hbt]
	...
	\label{classical}
\end{figure}
When looking at the predicted images, one notices that the structure of the images is still intact and it can be easily recognized what is depicted in the image. This is due to the fact that in \emph{Lab} color space the \emph{L}-layer displays most of the structure. The predictions for the colors are monotonous with a value of 131 for the \emph{a} channel and 136 for the \emph{b} channel. These two values are assigned to all the pixels throughout all images and correspond to a grayish sepia hue. There are neither differences in color between pixels nor images.\\
As one can see in figure \ref{loss_class}, both the training and the validation loss start with a very high value of around 17 000 for the training loss and 13 000 for the validation loss . Within the first epochs, they decrease rapidly and then converge to a value around 230 to 245. The validation loss is slightly lower as the training loss in the beginning.
\begin{figure}[h!]
	\centering
...
\begin{figure}
	...
	\label{classific}
\end{figure}
The first thing one might notice is that the originals look different in this approach. This is due to the fact that the color space was quantized to 121 different color values. This means all color values are distributed into bins and, hence, no gradual changes in color can be observed in the images. The predictions display the same grayish color hue throughout the image as in the classical approach. An \emph{a} and \emph{b} value of 125 is predicted for most pixels, but additional some pixels in the \emph{b} channel have a predicted value of 100. Those differently predicted pixels are organized in a grid of 14 by 14 dots. For the \emph{a} channel a uniform value of 125 is predicted for all pixels.
